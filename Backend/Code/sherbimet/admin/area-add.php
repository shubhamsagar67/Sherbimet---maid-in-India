...
//     $city_id = mysqli_real_escape_string($conn, $_POST['city_id']);
    ...
    $query_check = mysqli_query($conn, "select lower(area_name) from $table where area_name=lower('{$area_name}')") or die(mysqli_error($conn));
  ...
  if($count>0)
  {
    ...
    $queryins = mysqli_query($conn, "insert into $table(area_name,insert_datetime)values('{$area_name}','{$datetime}')") or die(mysqli_error($conn));
                  ...
                  <div class="row">
                    <div class="col-sm-3">
                      ...
                      <div class="form-group">
                        <label>Area</label>
                        <input type="text" name="area_name" onkeyup ="Validatestring(this)" class="form-control" placeholder="Enter Area" required="">
                      </div>
                    </div>
